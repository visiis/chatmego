...
<script>
    ...
    window.isProfileOwner = {{ Auth::id() == $user->id ? 'true' : 'false' }};
</script>
<div class="container">
    <div class="row">
        <div class="col-md-3">
                            ...
                            <div class="d-flex align-items-start">
                                ...
                                <div class="flex-grow-1">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <span class="text-muted text-sm">{{ $status->created_at->diffForHumans() }}</span>
                                    </div>
                                    @if($status->content)
                                    <p class="mt-2">{{ $status->content }}</p>
                                    @endif
                                    @if($status->images && count($status->images) > 0)
                                    <div class="row mt-3 status-images" data-status-id="{{ $status->id }}">
                                        @foreach($status->images as $index => $image)
                                        <div class="col-4 mb-2">
                                            <img src="{{ $image }}" 
                                                 ...
                                                 onclick="openStatusLightbox({{ $status->id }}, {{ $index }})">
                                        </div>
                                        @endforeach
                                    </div>
                                    @endif
                                    <div class="d-flex mt-3">
                                        <button class="btn btn-link text-muted" data-status-id="{{ $status->id }}" onclick="handleLikeBtn(this)">
                                            <i class="{{ $status->liked ? 'fas fa-heart text-danger' : 'far fa-heart' }}"></i>
                                            <span class="ml-1 like-count">{{ $status->likes_count }}</span>
                                            <span class="ml-1">点赞</span>
                                        </button>
                                        <button class="btn btn-link text-muted ml-4" data-status-id="{{ $status->id }}" onclick="handleCommentBtn(this)">
                                            <i class="far fa-comment"></i>
                                            <span class="ml-1">{{ $status->comments_count }}</span>
                                            <span class="ml-1">评论</span>
                                        </button>
                                        @if(Auth::id() == $status->user_id)
                                        <button class="btn btn-link text-muted ml-4" data-status-id="{{ $status->id }}" onclick="handleDeleteBtn(this)">
                                            <i class="far fa-trash-alt"></i>
                                            <span class="ml-1">删除</span>
                                        </button>
                                        @endif
                                    </div>
                                    <div class="comment-section mt-3" id="comments-{{ $status->id }}" style="display: none;">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control comment-input" placeholder="写下评论..." data-status-id="{{ $status->id }}">
                                            <div class="input-group-append">
                                                <button class="btn btn-primary" data-status-id="{{ $status->id }}" onclick="handleSendCommentBtn(this)">发送</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="card">
                                <a href="{{ route('album.show', ['userId' => $user->id, 'albumId' => $album->id]) }}">
                                    <img src="{{ $album->photos->count() > 0 ? ($album->photos->first()->thumbnail_url ?? $album->photos->first()->image_url) : '/images/default-album.svg' }}" 
                                         class="card-img-top" 
                                         alt="{{ $album->name }}"
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="card-text text-muted text-sm">{{ $album->photos->count() }} 张图片</p>
                                    @if($album->privacy)
                                ...
                                <div class="card-footer">
                                    <div class="d-flex gap-2">
                                        ...
                                        <button class="btn btn-xs btn-light flex-1" 
                                                ...
                                                data-album-privacy="{{ $album->privacy }}"
                                                data-album-price="{{ $album->price }}"
                                                onclick="showEditAlbumModal(this)">
                                            <i class="fas fa-edit"></i> 编辑
                                        </button>
                                        @endif
                                        <a href="{{ route('album.show', ['userId' => $user->id, 'albumId' => $album->id]) }}" class="btn btn-xs btn-primary flex-1">
                ...
                <div class="tab-pane fade" id="info-panel">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            @if($user->hobbies || $user->specialty || $user->love_declaration)
                            ...
                            <div class="border-top pt-4 mt-4">
                                ...
                                <div class="row mt-3">
                                    ...
                                    <div class="col-md-4 text-center">
                                        <div class="font-weight-bold">{{ $user->fans_count ?? 0 }}</div>
                        ...
                        <div class="card-body">
                            <form id="edit-profile-form" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="phone" class="form-control" id="phone" value="{{ $user->phone }}" placeholder="请输入手机号码">
                                </div>
                                
                                <div class="form-group">
                                    <label for="signature">个性签名</label>
                                    <input type="text" name="signature" class="form-control" id="signature" value="{{ $user->signature }}" placeholder="说说你的个性...">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="age" class="form-control" id="age" value="{{ $user->age }}" min="18" max="120">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="weight" class="form-control" id="weight" value="{{ $user->weight }}" min="30" max="300">
                                </div>
                                
                                <div class="form-group">
                                    <label for="hobbies">兴趣爱好</label>
                                    <input type="text" name="hobbies" class="form-control" id="hobbies" value="{{ $user->hobbies }}" placeholder="例如：音乐、阅读、运动...">
                                </div>
                                
                                <div class="form-group">
                                    <label for="specialty">特长技能</label>
                                    <input type="text" name="specialty" class="form-control" id="specialty" value="{{ $user->specialty }}" placeholder="你的特长或技能...">
                                </div>
                                
                                <div class="form-group">
                                    <label for="love_declaration">爱情宣言</label>
                                    <textarea name="love_declaration" class="form-control" id="love_declaration" rows="3" placeholder="写下你的爱情宣言...">{{ $user->love_declaration }}</textarea>
                                </div>
